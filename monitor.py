...
obtener_datos() {
    ...
    memoria=$(free | grep Mem | tr -s ' ' | cut -d ' ' -f 3,2)
    memoria_usada=$(echo "$memoria" | cut -d ' ' -f 1)
    memoria_total=$(echo "$memoria" | cut -d ' ' -f 2)
...
trap "echo -e '\nMonitoreo detenido por el usuario.'" SIGINT
while true; do
    obtener_datos
    sleep "$tiempo"
done
